...
class comp_filt:
    ...
    def attitude2(self,ax,ay,az,gx,gy,gz):
        ...
        k = self.pitch_d=math.degrees(self.pitch)#deg
        k2 = self.roll_d=math.degrees(self.roll)  #deg
        k3 = self.pitch_r=self.pitch              #rad
    ...
    def attitude3(self,ax,ay,az,gx,gy,gz,mx,my,mz):
       ...
       # Multiply error by kp gain
        ...
        if self.yaw > 2*math.pi:
            ...
            self.yaw=self.yaw+2*math.pi
    #    print "hello = ", self.pitch
        # Save pitch, roll, and yaw data for export in both radians and  degrees
        k=self.pitch_d=math.degrees(self.pitch)#deg
        k2=self.roll_d=math.degrees(self.roll)  #deg
        k3= self.yaw_d=math.degrees(self.yaw)    #deg
        k4=self.pitch_r=self.pitch              #rad
        k5=self.roll_r=self.roll                #rad
        k6=self.yaw_r=self.yaw                  #rad
